...
func TestMultiMatchSubqueryBuild(t *testing.T) {
	...
	expectedResult := `SELECT ({:mm},{:external}) as [[multiMatchValue]] FROM "test_FromTableName" "test_FromTableAlias" LEFT JOIN "join_table1" "join_alias1" ON TRUE LEFT JOIN "join_table2" "join_alias2" ON 123={:join} WHERE "test_FromTableAlias"."id" = "test_TargetTableAlias"."id"`
