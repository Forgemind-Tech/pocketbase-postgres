...
//
// The backup does not block writes. pg_dump reads a consistent snapshot of the
// database, and file deletions are postponed until the backup finishes so that
// the archived files cannot lose something the dump still references.
//
...
func (app *BaseApp) CreateBackup(ctx context.Context, name string) error {
	...
	// clears the marker and runs any file deletions postponed meanwhile
	defer app.endBackupWindow()
	...
	return app.OnBackupCreate().Trigger(event, func(e *BackupEvent) error {
		...
		// Dump the database alongside pb_data, since with Postgres the data no
		// longer lives inside the data directory.
		//
		// Nothing is blocked while this runs: pg_dump reads a consistent MVCC
		// snapshot, so concurrent writes neither affect it nor wait for it.
		//
		// The archived files are captured a moment later than the dump, which
		// is safe in one direction only. A file created in between ends up in
		// the archive without a matching record - a harmless orphan. A file
		// *deleted* in between would leave the dump referencing something the
		// archive no longer has, restoring as a broken attachment; that is
		// prevented by postponing file deletions for as long as the active
		// backup marker is set (see deferredDeletes).
		...
		//
		// note: upstream wrapped this in a transaction to block other writes
		// while the SQLite file inside pb_data was copied. The database is no
		// longer in there, so the pause bought nothing and has been dropped.
